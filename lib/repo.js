...
var publicProperties = ["path"],
  methods = [
    { name: 'commits', handlesError: true, transformer: function(commits){ if(commits) return commits.map(commit.wrap); } },
    ...
    { name: 'tags', handlesError: true, transformer: function(tags){ if(tags) return tags.map(tag.wrap); } },
    { name: 'create_tag', handlesError: false, transformer: null },
    { name: 'delete_tag', handlesError: false, transformer: null },
    { name: 'branches', handlesError: true, transformer: function(heads){ if(heads) return heads.map(head.wrap);} },
    { name: 'create_branch', handlesError: false, transformer: null},
    { name: 'delete_branch', handlesError: false, transformer: null},
    { name: 'branch', handlesError: true, transformer: head.wrap },
